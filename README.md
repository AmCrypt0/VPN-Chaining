...
**نکته**: در صورتی که پس از وارد کردن کد ششم (`ip route add default via 10.8.0.2 table 120 `)، ارور (`Error: Nexthop has invalid gateway`) دریافت کردید، مراحل زیر رو انجام بدین:
1. به سرور خارجی لاگین کنید و دستور زیر رو بزنید:
```bash
wget https://git.io/vpn -O openvpn-install.sh && bash openvpn-install.sh
```
2. عدد 3 رو بزنید تا ابزار `OpenVPN` به طور کامل از روی سرورتون پاک بشه.
3. دوباره دستور بالا رو بزنید تا `OpenVPN` وارد مراحل نصب بشه و این بار در قسمت وارد کردن پروتکل به جای عدد 443 از عدد دیگری (برای مثال 8642 یا پورت های غیر مرسوم دیگر) استفاده کنید و سپس ادامه مراحل را مطابق آموزش انجام دهید.
